...
template <int N, int M>
Matrix<N, M> Matrix<N, M>::column_op_vec(const Matrix<N, 1> &b, int pos,
                                         int op) {
  Matrix<N, M> res = *this;
  for (int i = 0; i < N; ++i) {
    if (op == 0)
...
template <int N, int M> Matrix<M, N> Matrix<N, M>::T() const {
  Matrix<M, N> res;
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < M; ++j)
      res[j][i] = mat[i][j];
...
template <int N, int M> Matrix<N, M> Matrix<N, M>::inv() {
  assert(det() != 0);
  auto x = mat, ob = mat;
  for (int i = 0; i < N; ++i)
    for (int j = 0; j < N; ++j) {
...
template <int N, int M> Matrix<min(N, M), 1> Matrix<N, M>::get_diag() {
...
template <int N, int M> Matrix<N, 1> Matrix<N, M>::get_column(int num) {
  Matrix<N, 1> res;
  for (int i = 0; i < N; ++i)
    res[i] = mat[i][num];
  return res;
}
template <int N, int M> Matrix<M, 1> Matrix<N, M>::get_row(int num) {
  Matrix<M, 1> res;
  for (int i = 0; i < M; ++i)
    res[i] = mat[num][i];
  return res;
}
template <int N, int M> double Matrix<N, M>::det() {
  assert(N == M);
  auto x = mat;
  for (int k = 0; k < N - 1; k++) {
    for (int i = k + 1; i < N; i++) {
...
template <int N, int M> void Matrix<N, M>::printer() const {
...
template <int N>
Matrix<N, 1> make_operation(const Matrix<N, 1> &a, const Matrix<N, 1> &b,
                            int op) {
  Matrix<N, 1> res;
  for (int i = 0; i < N; ++i) {
    if (op == 0)
...
template <int N>
Matrix<1, N> make_operation(const Matrix<1, N> &a, const Matrix<1, N> &b,
                            int op) {
