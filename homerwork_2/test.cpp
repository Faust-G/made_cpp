...
template <int N> bool compare(const Matrix<N, 1> &a, const Matrix<N, 1> &b) {
  for (int i = 0; i < N; ++i)
    if (abs(a[i] - b[i]) > 1e-5)
      return false;
  return true;
...
template <int M> bool compare(const Matrix<1, M> &a, const Matrix<1, M> &b) {
  for (int i = 0; i < M; ++i)
    if (abs(a[i] - b[i]) > 1e-5)
      return false;
  return true;
...
TEST(Matrix_n_m, Test_row_vec_operations) {
  ...
  auto res = Matrix<2, 3>(
      {array<double, 3>{2, 3, 7}, array<double, 3>{4 + 3, 6 - 2, 9 + 6}});
  EXPECT_EQ(compare(res, mat.sum_row_vec(x, 1)), true);
  res = Matrix<2, 3>(
      {array<double, 3>{2 - 3, 3 - -2, 7 - 6}, array<double, 3>{4, 6, 9}});
...
TEST(Matrix_n_m, Test_det_inv) {
  auto x = Matrix<3, 3>({array<double, 3>{2, 5, 7}, array<double, 3>{6, 3, 4},
                         array<double, 3>{5, -2, -3}});
  auto inv_x =
      Matrix<3, 3>({array<double, 3>{1, -1, 1}, array<double, 3>{-38, 41, -34},
...
TEST(Matrix, Test_T) {
  auto res = Matrix<3, 2>(
      {array<double, 2>{2, 4}, array<double, 2>{3, 6}, array<double, 2>{7, 9}});
...
TEST(Matrix, Mat_Mult) {
  auto x = Matrix<3, 3>({array<double, 3>{2, 5, 7}, array<double, 3>{6, 3, 4},
                         array<double, 3>{5, -2, -3}});
  auto E = Matrix<3, 3>({array<double, 3>{1, 0, 0}, array<double, 3>{0, 1, 0},
